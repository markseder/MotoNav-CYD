...
enum UiMode { DRIVE_UI, MENU_UI, SETTINGS_UI, SUMMARY_UI };
...
bool saveRideStatistics() {
  ...
  stats.println("gpx_file,distance_km,total_seconds,moving_seconds,stopped_seconds,average_kmh,maximum_kmh,gpx_points");
  stats.printf("%s,%.3f,%lu,%lu,%lu,%.2f,%.2f,%lu\n",
               summaryTrackFileName.c_str(),
               ...
               summaryAverageSpeedKmh,
               summaryMaximumSpeedKmh,
               static_cast<unsigned long>(summaryPointCount));
  stats.flush();
  const bool ok = !stats.getWriteError();
  stats.close();
  return ok;
}
...
void setup() {
  ...
  delay(250);
  Serial.println();
  Serial.println("MotoNav-CYD V0.8 Trip Computer");
